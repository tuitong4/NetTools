...
automatically.`)
    ...
    flag.IntVar(&args.transporttime, "transporttime", 300, "Time of uploading file complete, in second.")
    flag.Parse()
}
func h3c_upload(host, port string, sshoptions nwssh.SSHOptions, args Args) (string, bool) {
    ...
    output += o
    o, err = device.ExecCommandExpect(args.ftpuser, "assword:", time.Second*5)
    if err != nil {
        log.Printf("[%s]%v\n", host, err)
        return output + o, false
    }
    ...
    output += o
    o, err = device.ExecCommandExpectPrompt("get "+args.filename, time.Second*time.Duration(args.transporttime))
    if err != nil {
        log.Printf("[%s]%v\n", host, err)
        return output + o, false
    }
...
func huawei_upload(host, port string, sshoptions nwssh.SSHOptions, args Args) (string, bool) {
    ...
    ftp_cmd := ""
    if args.vpn != "" {
        ftp_cmd = `ftp -a ` + host + ` ` + args.ftpserver + ` vpn-instance ` + args.vpn
    } else {
        ftp_cmd = `ftp -a ` + host + ` ` + args.ftpserver
    }
    ...
    output += o
    o, err = device.ExecCommandExpect(args.ftpuser, "assword:", time.Second*25)
    if err != nil {
        log.Printf("[%s]%v\n", host, err)
        return output + o, false
    }
    output += o
    o, err = device.ExecCommandExpect(args.ftppassword, "ftp]", time.Second*25)
    if err != nil {
        log.Printf("[%s]%v\n", host, err)
        return output + o, false
    }
    ...
    output += o
    o, err = device.ExecCommandExpect("get "+args.filename, "ftp]", time.Second*time.Duration(args.transporttime))
    if err != nil {
        log.Printf("[%s]%v\n", host, err)
        return output + o, false
    }
...
func main() {
    ...
    for _, host := range hosts {
        ...
        go func(host string) {
            ...
            output := ""
            if  vendor == "h3c" {
                output, _ = h3c_upload(host, "22", sshoptions, args)
            } else if vendor == "huawei" {
                output, _ = huawei_upload(host, "22", sshoptions, args)
            } else{
